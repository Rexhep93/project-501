...
def find_csv(base_path, name):
    ...
    for f in base_path.rglob(f"*{name}*"):
        if f.suffix == ".csv":
            return f
    # Soms zitten ze in submappen met dezelfde naam
...
def load_performances(base_path):
    ...
    # Lees in chunks om geheugen te besparen
    players = {}
    chunk_size = 100_000
    total_rows = 0
...
def build():
    ...
    for pid, perf in performances.items():
        if perf["career"]["apps"] < MIN_APPEARANCES:
            skipped_low += 1
            continue
        ...
        player = {
            ...
            # Career
            "ca": perf["career"]["apps"],
            "cg": perf["career"]["goals"],
            "cas": perf["career"]["assists"],
    ...
    # Categorieën
    categories = [
        {"id": "career_goals", "label": "Career Goals", "desc": "Totaal goals in clubcarrière", "stat": "cg"},
        {"id": "career_apps", "label": "Career Appearances", "desc": "Totaal wedstrijden in clubcarrière", "stat": "ca"},
    ...
    # Voorbeelden
    print("\n🔍 Top 5 spelers (by appearances):")
    for p in game_players[:5]:
        print(f"  {p['n']} — {p['ca']} apps, {p['cg']} goals, {p['cas']} assists, {p['ic']} caps")
